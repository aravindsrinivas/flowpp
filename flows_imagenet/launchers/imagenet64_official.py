"""
Num params: 73489960
Run it as: mpiexec -n {num_processes} python3.6 -m flows_imagenet.launchers.imagenet64_official from the flows master directory of the git repo. 
           num_processes=6 was used for this launcher on a DGX-1 with 8 V100 GPUs and 52GB CPU RAM.  
...
def main():
    ...
    flow_training_imagenet.train(
        ...
        logdir=f'~/logs/2018-11-12/imagenet64_ELU_code_release_mix{components}_b{blocks}_f{filters}_h{attn_heads}_ln{int(use_ln)}_lr{max_lr}_bs{bs}_drop{dropout_p}_{floatx_str}',
        lr_schedule=lr_schedule,
        dropout_p=dropout_p,
        seed=0,
        init_bs=72, # set this to a smaller value if it can't fit on your GPU. This setting works for a V100 but for smaller GPUs, batch size of 56 is advised.
        dataset='imagenet64',
        total_bs=bs,
        ema_decay=.999,
        steps_per_log=100,
